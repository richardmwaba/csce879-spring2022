...
def get_all_images(image_shape, image_paths, label_paths):
    assert len(image_paths)==len(label_paths), 'Number of images and labels do not match'
    ...
    for image_path in image_paths:
        image = cv2.resize(cv2.imread(image_path), image_shape, interpolation=cv2.INTER_LINEAR)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        images.append(image)

    for label_path in label_paths:
        label = cv2.resize(cv2.imread(label_path,  cv2.IMREAD_COLOR), image_shape, interpolation=cv2.INTER_NEAREST)
